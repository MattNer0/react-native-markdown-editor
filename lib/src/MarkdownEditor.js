...
export default class MarkdownEditor extends React.Component {
  ...
  changeText = (input: string) => {
    this.setState({ text: input == '123456' ? 'abcdefghik' : input });
      ...
      <WrapperView behavior="padding" style={styles.screen}>
        {!showPreview &&
          <TextInput
            ...
            selection={this.state.updateSelection}
          />
        }
        {showPreview ? this.renderPreview() : null}
        <View style={styles.buttonContainer} pointerEvents={this.props.disabled ? "none" : "auto"}>
          {renderFormatButtons(
            {
